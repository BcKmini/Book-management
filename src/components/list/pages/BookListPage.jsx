...
export default function BookListPage({ onClickNew, onClickBook }) {
  ...
  const filtered = books.filter((book) => {
    const genreOk = genre === '전체' || (genre === FAVORITES ? favoriteIds.has(String(book.id)) : book.genre === genre)
    const lowerQuery = query.toLowerCase()
    const queryOk = !query || book.title?.toLowerCase().includes(lowerQuery) || book.author?.toLowerCase().includes(lowerQuery)
      ...
      <div style={styles.main}>
        ...
        <div style={styles.content}>
          ...
          {!loading && !error && view === 'list' && (
            <div>
              {filtered.map((book, i) => (
                <BookListItem key={book.id} book={book} rank={i + 1} onClick={() => onClickBook?.(book)} onDelete={() => handleDelete(book.id)} />
              ))}
            </div>
          )}
        </div>
